...
typedef struct {
    ...
    uint8_t* data;
    /** The current and maximum page count for this Memory object. If there is
     * no maximum, `max_pages` is 0xffffffffu (i.e. UINT32_MAX). */
...
typedef void* wasm_rt_func_handle_t;
...
typedef struct {
     ...
     *    wasm_rt_register_func_type(2, 1, WASM_RT_I32, WASM_RT_F32,
     * WASM_RT_I64);
     *    => returns 1
     *
     ...
     *    => returns 1
     *  ``` */
    uint32_t (*wasm_rt_register_func_type)(void* context, uint32_t params,
                                           uint32_t results, ...);

    /** Initialize a Memory object with an initial page size of `initial_pages`
     * and a maximum page size of `max_pages`.
     *
     *  ```
     ...
     *  ``` */
    void (*wasm_rt_allocate_memory)(void* context, wasm_rt_memory_t*,
                                    uint32_t initial_pages, uint32_t max_pages);
     ...
     * fails and 0xffffffffu (UINT32_MAX) is returned instead.
     *
     *  ```
     *    wasm_rt_memory_t my_memory;
     *    ...
     ...
     *    }
     *  ``` */
    uint32_t (*wasm_rt_grow_memory)(void* context, wasm_rt_memory_t*,
                                    uint32_t pages);

    /** Initialize a Table object with an element count of `elements` and a
     * maximum page size of `max_elements`.
     *
     *  ```
     ...
     *  ``` */
    void (*wasm_rt_allocate_table)(void* context, wasm_rt_table_t*,
                                   uint32_t elements, uint32_t max_elements);

    /* 根据size分配一段内存大小 */
    void* (*wasm_rt_malloc)(void* context, uint32_t size);

    /* 根据module和name返回一个后续可以用于wasm_rt_call_func的handle */
    wasm_rt_func_handle_t (*wasm_rt_resolve_func)(void* context, char* module,
                                                  char* name);
    /* 根据一个function handle发起一个函数的调用 */
    uint32_t (*wasm_rt_call_func)(void* context, wasm_rt_func_handle_t hfunc,
