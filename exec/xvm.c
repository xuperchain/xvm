...
static bool func_types_are_equal(struct FuncType* a, struct FuncType* b) {
    ...
    for (i = 0; i < a->result_count; ++i)
        if (a->results[i] != b->results[i]) return 0;
    return 1;
}
...
static uint32_t wasm_rt_register_func_type(void* context, uint32_t param_count,
                                           uint32_t result_count, ...) {
    ...
    code->func_types = xvm_realloc(
        code->func_types, code->func_type_count * sizeof(struct FuncType));
...
static void wasm_rt_allocate_table(void* context, wasm_rt_table_t* table,
...
static wasm_rt_func_handle_t wasm_rt_resolve_func(void* context, char* module,
                                                  char* name) {
    xvm_code_t* code = context;
    return code->resolver.resolve_func(code->resolver.env, module, name);
}

static uint32_t wasm_rt_call_func(void* context, wasm_rt_func_handle_t hfunc,
...
static int64_t wasm_rt_resolve_global(void* context, char* module, char* name) {
    xvm_context_t* ctx = context;
    return ctx->code->resolver.resolve_global(ctx->code->resolver.env, module,
                                              name);
}
...
uint32_t xvm_call(xvm_context_t* ctx, char* name, int64_t* params,
                  int64_t param_len, int64_t* ret) {
